...
namespace OculusRiftLib
{
    ...
    {
        private readonly Matrix[] _projectionMatrix = new Matrix[2]; // one for each eye
        ...
        public int Init(GraphicsDevice gd)
        {
            ...
            if ((result = NativeRift.CreateDXSwapChains(recommendTexResLeft, recommendTexResRight, fovLeft, fovRight)) <
                0)
                return result;

            for (var eye = 0; eye < 2; eye++)
                _projectionMatrix[eye] = NativeRift.GetProjectionMatrix(eye, 0.1f, 1000, 0);
        ...
        {
            return Matrix.Transpose(_projectionMatrix[eye]);
        }
